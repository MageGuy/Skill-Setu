...
// Auto-schedule via node-cron if enabled
function scheduleAutoCheckIn() {
  ...
  try {
    ...
    // 1st of Jan, May, September at 09:00 AM
    // Cron format: minute hour day-of-month month day-of-week
    const task = cron.schedule('0 9 1 1,5,9 *', async () => {
      console.log('\n📅 Running scheduled quarterly check-in cycle...');
      try {
    ...
    console.log('📅 Quarterly check-in cron scheduled (1st of Jan/May/Sep at 09:00)');
    return task;
  } catch (err) {
    console.warn('⚠️  node-cron not installed — auto-scheduling disabled. Run manually.');
    return null;
  }
}
